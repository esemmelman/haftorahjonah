...
function missingRecordingGroups() { return groups.filter(group => group.verse <= VERSES.length && !recordings.has(group.id)); }
...
async function loadRemoteState() {
  try {
    const [groupResponse, recordingResponse] = await Promise.all([
      fetch(`${SUPABASE_URL}/rest/v1/${GROUP_TABLE}?verse=lte.${VERSES.length}&select=id,verse,start_word,end_word,color&order=id.asc`, { headers: apiHeaders() }),
